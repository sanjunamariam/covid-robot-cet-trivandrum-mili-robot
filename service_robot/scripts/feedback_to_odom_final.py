...
linear_scale_positive = 1
linear_scale_negative = 1
angular_scale_positive = 1
angular_scale_negative = 1
...
## modification for time
    clock = Clock()
    clock_pub = rospy.Publisher('clock',Clock, queue_size=50)
    while not rospy.is_shutdown():
	    ...
	    #about odom frame
            ...
            #################################  
            dxy = actual_vehicle_linear_vel * time_diff
	  
            if (dxy > 0):
		dxy *= linear_scale_positive
            if (dxy < 0):
		dxy *= linear_scale_negative

            if (dtheta > 0):
		dtheta *= angular_scale_positive
            if (dtheta < 0):
		dtheta *= angular_scale_negative
            ############# added to solve poor odometry ####################

	    dx = dxy * cos(actual_theta)
	    dy = dxy * sin(actual_theta)
